...
# CopyFactory — leave empty on first deploy; run /api/admin/copyfactory/strategy (POST) to create it.
# The strategy ID is then persisted in the DB (SystemSetting) and served to subscribers via /api/copyfactory/strategy-info
