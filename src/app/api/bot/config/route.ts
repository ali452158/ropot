  ...
  try {
    ...
    const cfg = await db.botConfig.upsert({
      ...
      create: {
        ...
        autoPairScan: patch.autoPairScan ?? true,
        scanSymbols: patch.scanSymbols ?? "XAUUSD,EURUSD,GBPUSD,USDJPY,AUDUSD,USDCAD,XAGUSD",
        ...
        maxTradeMinutes: patch.maxTradeMinutes ?? 30,
        botRunning: false,
      },
    });
    return NextResponse.json({ ok: true, config: cfg });
